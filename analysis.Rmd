...
I use random forests algorithm to obtain the prediction model, using all variables as predictors of **Classe**. This selection was based on the high accuracy of this algorithm and the size of training data, despite its speed and possible overfitting.

```{r cache=TRUE}
model <- randomForest(classe~., data=training.data)
print(model)
```

I observe from model obtained a very high accuracy, so the expection was to predict correctly the 20 test cases provided.
